      ...
      <div class="container">
        <div class="row">
          <div class="tittle col text-white">
            <p>Girimulyo > Statistik > Usia</p>
            <h1 class="fw-bold">Statistik Usia Penduduk Desa Girimulyo</h1>
          </div>
        </div>
      </div>
    </section>
...
<script>
    ...
    const eduTable = document.querySelectorAll('.usia')
    const kelompok = [];
    const jumlah = [];

    eduTable.forEach(dataUsia => {
        const iterasi = dataUsia.children.length;
        for(let i=0; i<iterasi; i++){
            if(i == 1) kelompok.push(htmlDecode(dataUsia.children[i].innerHTML))
            else if(i == 2) jumlah.push(dataUsia.children[i].innerHTML)
        }
    });
