      ...
      <div class="container">
        <div class="row">
          <div class="tittle col text-white">
            <p>Girimulyo > Statistik > Pekerjaan</p>
            <h1 class="fw-bold">Statistik Pekerjaan Penduduk Desa Girimulyo</h1>
          </div>
        </div>
      </div>
    </section>
...
<script>
    const kerjaTable = document.querySelectorAll('.kerja')
    const kelompok = [];
    const jumlah = [];

    kerjaTable.forEach(dataKerja => {
        const iterasi = dataKerja.children.length;
        for(let i=0; i<iterasi; i++){
            if(i == 1) kelompok.push(dataKerja.children[i].innerHTML)
            else if(i == 2) jumlah.push(dataKerja.children[i].innerHTML)
        }
    });
