      ...
      <div class="container">
        <div class="row">
          <div class="tittle col text-white">
            <p>Girimulyo > Statistik > Vaksinisasi</p>
            <h1 class="fw-bold">Statistik Vaksinisasi Penduduk Desa Girimulyo</h1>
          </div>
        </div>
      </div>
    </section>
      ...
      <div class="col-lg-7 col-12 d-flex flex-column">
        ...
        <table class="table table-striped table-hover">
            <thead>
                <tr>
                <th scope="col">No</th>
                <th scope="col">Kelompok Vaksin</th>
                <th scope="col">Jumlah</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($datavaksin as $pek )
                <tr class="vaksin">
...
<script>
    ...
    const vacineTable = document.querySelectorAll('.vaksin')
    const kelompok = [];
    const jumlah = [];

    vacineTable.forEach(datavaksin => {
        const iterasi = datavaksin.children.length;
        for(let i=0; i<iterasi; i++){
            if(i == 1) kelompok.push(datavaksin.children[i].innerHTML)
            else if(i == 2) jumlah.push(datavaksin.children[i].innerHTML)
        }
    });
